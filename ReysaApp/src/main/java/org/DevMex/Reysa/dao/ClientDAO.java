...
public class ClientDAO {
    ...
    public static int parseVehicleId(String idStr) {
        if (idStr == null) return 0;
        try {
            if (idStr.startsWith("ORDEN-2026-")) {
                return Integer.parseInt(idStr.substring("ORDEN-2026-".length()));
            }
    ...
    public static String formatVehicleId(int id) {
        return String.format("ORDEN-2026-%03d", id);
    }
    ...
    public boolean registrarTodo(Client cliente, Vehicle vehiculo) {
        ...
        try {
            ...
            try (PreparedStatement psV = con.prepareStatement(sqlVehiculo)) {
                psV.setInt(1, parseVehicleId(vehiculo.getId()));
    ...
    public boolean actualizarEstado(String idVehiculoStr, String nuevoEstado) {
        int idVehiculo = parseVehicleId(idVehiculoStr);
    ...
    public boolean eliminarVehiculoCompleto(String idVehiculoStr) {
        int idVehiculo = parseVehicleId(idVehiculoStr);
        ...
        try (Connection con = Conexion.getConexion()) {
            ...
            try (PreparedStatement ps = con.prepareStatement(sql);
                ...
                while (rs.next()) {
                    ...
                    String idStr = formatVehicleId(id);
                    ...
                    lista.add(new Vehicle(idStr, marca, modelo, color, placa, vin, state));
                }
            }
        } catch (SQLException e) {
            e.printStackTrace();
